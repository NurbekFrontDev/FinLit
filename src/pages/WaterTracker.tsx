...
// Трекер питьевой воды (💧). Вдохновлён MapMyRun water tracker.
// Сверху: горизонтальный слайдер объёмов (150-600 мл шагами).
// Ниже: SVG-кольцо прогресса + визуальные стаканы (тап = добавить/убрать).
// Настройка цели (шестерёнка справа), история за неделю.
...
export default function WaterTracker() {
  ...
  const shiftHist = (by: number) => { setHistOffset((o) => o + by); reload() }
    ...
    <div className="mx-auto flex max-w-2xl flex-col gap-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            type="button"
            onClick={() => { setGoalEdit(!goalEdit); setGoalDraft(String(goal)) }}
            className="cursor-pointer rounded-lg border border-neutral-300 px-3 py-1.5 text-xs font-medium transition hover:bg-neutral-100 dark:border-neutral-700 dark:hover:bg-neutral-800"
          >
            ⚙️
          </button>
      ...
      {loading ? (
        ...
        <div className={`${cardCls} flex flex-col gap-4`}>
          <div className="flex items-center justify-between">
            <button onClick={() => shiftHist(-1)} className="cursor-pointer rounded px-2 py-1 text-sm transition hover:bg-neutral-100 dark:hover:bg-neutral-800">‹</button>
            <span className="text-sm font-medium">{histDays[0]} – {histDays[6]}</span>
            <button onClick={() => shiftHist(1)} className="cursor-pointer rounded px-2 py-1 text-sm transition hover:bg-neutral-100 dark:hover:bg-neutral-800">›</button>
          </div>
                ...
                <div key={d} className="flex flex-1 flex-col items-center gap-1">
                  ...
                  <div className="flex w-full flex-1 flex-col justify-end">
                    <div
                      ...
                      style={{ height: `${Math.max(2, dayPct)}%`, minHeight: ml > 0 ? 8 : 0 }}
                    />
                  </div>
        ...
        <>
          ...
          <div className={`${cardCls} flex flex-col gap-3`}>
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-neutral-500 dark:text-neutral-400">{t('water.quickAdd')}</p>
              <span className="text-lg font-semibold text-sky-600 dark:text-sky-400">{selMl} ml</span>
            </div>
            <div className="flex gap-1.5 overflow-x-auto pb-1">
              {QUICK_STEPS.map((ml) => (
                <button key={ml} type="button" onClick={() => setSelMl(ml)}
                  className={`shrink-0 cursor-pointer rounded-xl px-4 py-2.5 text-sm font-medium transition ${
                    selMl === ml ? 'bg-sky-500 text-white shadow-md' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200 dark:bg-neutral-800 dark:text-neutral-300'
                  }`}
                >{ml}</button>
              ))}
            </div>
            <button type="button" onClick={() => quickAdd(selMl)}
          ...
          <div className={`${cardCls} flex flex-col items-center gap-5`}>
            <div className="relative">
              <svg className="h-56 w-56 -rotate-90" viewBox="0 0 240 240">
                ...
                <circle cx="120" cy="120" r={R} fill="none" strokeWidth="14" className="text-neutral-200 dark:text-neutral-800" stroke="currentColor" />
                <circle cx="120" cy="120" r={R} fill="none" strokeWidth="14" strokeLinecap="round" stroke="url(#waterGrad)" strokeDasharray={C} strokeDashoffset={dashOffset} className="transition-[stroke-dashoffset] duration-700" />
              </svg>
              <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                <p className="text-4xl font-bold">{pct}%</p>
                <p className="text-sm text-neutral-500 dark:text-neutral-400">{drunk} / {goal} ml</p>
              </div>
            </div>

            <div className="flex flex-wrap justify-center gap-2">
              {Array.from({ length: glasses }).map((_, i) => (
                <button key={i} type="button" onClick={() => toggleGlass(i)}
                  className={`cursor-pointer text-3xl transition active:scale-90 ${
                    i < drunkGlasses ? 'opacity-100 drop-shadow-md' : 'opacity-30 hover:opacity-50'
                  }`}
                >🥛</button>
              ))}
            </div>
            ...
            {isFull && (
              <p className="text-sm font-semibold text-sky-600 dark:text-sky-400">{t('water.goalReached')}</p>
            )}
          </div>
          ...
          {goalEdit && (
            <div className={`${cardCls} flex flex-col gap-2`}>
              <p className="text-sm font-medium text-neutral-500 dark:text-neutral-400">🎯 {t('water.dailyGoal')}</p>
              <div className="flex gap-2">
                ...
                <button type="button" onClick={saveGoal} className="cursor-pointer rounded-lg bg-sky-400 px-3 py-1.5 text-sm font-medium text-white transition hover:bg-sky-300">{t('common.save')}</button>
                <button type="button" onClick={() => setGoalEdit(false)} className="cursor-pointer rounded-lg border border-neutral-300 px-3 py-1.5 text-sm transition hover:bg-neutral-100 dark:border-neutral-700 dark:hover:bg-neutral-800">{t('common.cancel')}</button>
              </div>
            </div>
          )}
          ...
          {day && day.logs.length > 0 && (
            <div className={`${cardCls} flex flex-col gap-2`}>
              <p className="text-sm font-medium text-neutral-500 dark:text-neutral-400">{t('water.todayLogs')}</p>
              {day.logs.map((log) => (
                <div key={log.id} className="flex items-center justify-between gap-3 rounded-lg bg-neutral-50 px-3 py-2 dark:bg-neutral-900/40">
                  <span className="text-sm">+{log.amount} ml</span>
                  <button type="button" onClick={async () => {
                    if (!user) return
                    await removeWaterLog(user.id, log.id)
                    setDay((prev) => prev ? { ...prev, drunk: Math.max(0, prev.drunk - log.amount), logs: prev.logs.filter((l) => l.id !== log.id) } : null)
